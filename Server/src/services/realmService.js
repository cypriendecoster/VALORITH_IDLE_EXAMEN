  ...
  try {
    ...
    if (costs.length === 0) {
      throw createError({
        ...
        message: 'Déblocage indisponible pour ce royaume.',
        status: 500
      });
    }
    ...
    throw createError({
      code: 'REALM_UNLOCK_FAILED',
      message: 'Impossible de débloquer le royaume pour le moment.',
      status: 500
    });
  }
}
...
export async function activateRealm(userId, realmId) {
  try {
    ...
    throw createError({
      code: 'REALM_ACTIVATE_FAILED',
      message: 'Impossible d’activer le royaume pour le moment.',
      status: 500
    });
  }
}
